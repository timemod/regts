...
The length of the range may be obtained by using the `length_range` function:
...
## Write timeseries to file via data frame 

The most convenient way to store `regts` timeseries in a file or data is to first convert the data to a data frame using the `as.data.frame` function. The period and names of the `regts` become automatically the `rownames` and `colnames` of the `data.frame`.
...
The most common used filetypes to store data are a `csv`, `xls`, `xlsx` or `.Rdata` file. 
The `.Rdata` file, an R-readable binary file, is the most flexible type, multiple objects can be stored 
(including one's entire current workspace).

For each of these filetypes there are methods available to save a data frame to a file: 

* `csv file`         : `write.csv` or `write.csv2` in package `utils` (installed by default)
* `xls and xlsx file`: several packages like `writeXLS` or `xlsx`
...
It is possible to read timeseries from different types of files. As we have seen the most common filetypes used in R are `csv`, `xls`, `xlsx`, and `Rdata`. When you want to read timeseries data from a file it is best to use the `data.frame` class as a means to do so.

### .Rdata file
Data saved in an `.Rdata` file by the `save` function can be reloaded  by the `load` function. By default they are loaded in the workspace. 
...
### Csv file
There are several ways to read a data frame from a csv file, like 
`read.csv` in package `utils` (installed by default) or `fread` in package `data.table`.
...
Note that in the file the quarter "2010Q4" is missing.

When reading information from this `csv file`, use `stringsAsFactors = FALSE` to prevent that character data (such as variablenames) is converted to factors.
...
Sometimes the data in the file are transposed, e.g. periods in the column headers and column names as rowheaders.
...
The periods in the columnheaders are no valid names, valid names should start with [a-z].
Therefore, when reading the file, no check must be performed on the names. To avoid this check use argument `check.names`: 
...
For reading information from an `xls` or an `xlsx` file for instance the function `read_excel` in package `readxl` is available.

```{r eval = FALSE}
df <- read_excel("test.xlsx")
```
When reading xls(x) files, character data (such as variablenames) are not converted, e.g. to factors. Unlike csv files there is no use for an argument like  `stringsAsFactors = FALSE`.

The transformation to a `regts` timeseries or reading a transposed file is similar to csv files. See the previous section.
...
Labels are no standard attribute of a data frame. The `label` function in the `Hmisc` package returns the label attribute of any R object. More interesting: `label` can be used specifically for the columns of a data frame. 

If a regts with labels is converted to a data frame the labels are preserved (via the Hmisc package). You can view a data frame including labels with the `View` function in Rstudio. If you just want to see the labels use the `label` function:
...
label(df)
```
With the `label` function you can assign a label to any R object, including a data frame:

```{r}
label(t1) <- "Label t1"
label(df, self = TRUE) <- "Data frame df"
```
And it is possible to set labels on the columns of a data frame:
...
Input for the labels in this function is a named character vector. The label vector is fetched from the `dflabel` variable and the names can be set with the `names` function:
...
labels
```
The variable `labels` contains more labels than we need. Also the ordering of the labels is not the same as in the  columns names of the regts object. This is not a problem, the function `update_ts_labels` takes care of all that:
...
The result is a list with components, starting with the result `equal`, which is `TRUE` or `FALSE`. 
...
The quarters in 2016 add up to 10, the quarters in 2017 to 26.    
Note that the 2 quarters in 2018 are ignored. The `aggregate` function skips the incomplete years at the end.
...
For this case the function `aggregate_gr` is developed with special aggregation algorithms, the so called "cgrx"-methods.^["cgr" stands for cumulative growth rate, the "x" is a qualifying postfix.] 
A more detailed description of these methods can be found in the next paragraph.

...
#' Alternatively, it is possible to specify a format employed by the base R
#' function \code{\link[base]{strptime}}, e.g. \code{"\%Y-\%m-\%d"}.
...
write_ts_csv <- function(
  ...
  if (verbose) {
    ...
    secs <- t_end - t_start
    message(sprintf(
      paste("%d timeseries written, period range %s, %.2f sec.", "elapsed.\n"),
      ncol(x),
      get_period_range(x),
      secs
    ))
...
#' or [openxlsx::loadWorkbook].
#'
#' The functions employ package [openxlsx] for writing the Excel file.
#'
...
#'                    returnValue = TRUE)
#' if (!ok) {
#'   stop("Failed to save workbook to file 'timeseries.xlsx'. Check warnings.")
#' }
#'
...
write_ts_xlsx <- function(
  ...
  if (!is.na(max_col_width)) {
    options(openxlsx.maxWidth = max_col_width)
  }
...
write_ts_sheet_ <- function(
  ...
  freezePane(wb, sheet, firstActiveRow = row_split, firstActiveCol = col_split)
...
ts2df_ <- function(
  ...
  if (rowwise) {
    ...
    column_headers <- as.data.frame(
      c(as.list(colnames(data)[1:n_rowheaders]), as.list(periods)),

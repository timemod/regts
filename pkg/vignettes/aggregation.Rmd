...
For growth timeseries the function `aggregate_gr` can be used. It accepts both `regts` and `ts` timeseries. The function implements several methods (`difmean`, `difsum`, `rel` and `pct`) for different types of input timeseries.

Methods `difmean` and `difsum` assume that the input timeseries is a first 
difference: 
\begin{displaymath}
 x_{t} = z_{t} - z_{t-1}
\end{displaymath}
where $t$ is the period (like year or quarter). These methods calculate the low frequency first difference timeseries. For method `difmean` the result is also scaled.
An example for both methods, the results are annual timeseries:

```{r}
xdif <- diff(regts(1:8, start = "2019q1"))
xdif
aggregate_gr(xdif, method = "difsum")
aggregate_gr(xdif, method = "difmean")  # result is also scaled
```

The `rel` and `pct` methods assume that the input timeseries is a one-period 
relative or percentage change and calculate the exact low frequency relative or percentage change.
The methods are described in detail in the next section.

# Aggregation methods for growth timeseries
This section describes the methods implemented in function `aggregate_gr`, including the derivation of the corresponding mathematical formulas. 

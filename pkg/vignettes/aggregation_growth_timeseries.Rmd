...
For growth timeseries the function `aggregate_gr` can be used. It accepts both `regts` and `ts` timeseries. The function implements several methods (`dif1`, `dif1s`, `rel` and `pct`) for different types of input timeseries.
...
\begin{displaymath}
 x_{t} = z_{t} - z_{t-1}
\end{displaymath}
where $t$ is the period (like year or quarter). These methods calculate the low frequency first difference timeseries. For method `dif1s` the result is also scaled.
...
## `dif1s` method 
The `dif1s` method is similar to the `dif1` method described in the previous 
...
The result for `dif1s` is the result for `dif1` divided by $n$:
\begin{displaymath}

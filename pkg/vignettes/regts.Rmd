...
```{r}
ts_data <- regts(1:10, prd_range = "2016Q1/2016Q4")
```
Input for this period may be a character string as shown above, but also a 
variable of type  `period_range`. This type will be explained in section \ref{sec:period}.
...
# `period` and `period_range`\label{sec:period}
...
rts <- regts(matrix(1:18, ncol = 3), prd_range = range1, names = c("a", "b", "c"))
rts
```
The length of a `period_range` may be obtained by using the `nperiod` function:
...
ts <- regts(data, prd_range = range2)
```

...
`NAs` are filled in for the missing periods.
...
By default labels are read from columns between the names column and the data columns. Use argument `labels = no` to prevent this.
...
## Period vectors
Two functions are available for creating a vector of consecutive periods: `get_periods` and `seq`.
Function `get_periods` returns a period vector of all periods in a range or
timeseries:
```{r}
get_periods(period_range("2019.1q", "2020.1q"))
```
...
Unfortunately a period vector cannot be used directly in a for-loop. First convert the vector, e.g. to a list or to a character vector.

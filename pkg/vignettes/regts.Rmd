...
Sometimes it is not possible to use the common functions (`read_ts_csv` and `read_ts_xlsx`) for reading a timeseries from a file. The information in the file deviates too much. In that case try reading the information as a form of `data frame` and convert this data frame to a `regts` timeseries. An example file:
...
Problem in this file are the extra comment lines with label information.
...
Select rownumbers^[For selecting a single column by position from a `tibble`  `[[`  is used.] with proper labels (not NA) and then select the rows. Multiply the result, each label is used for 3 variables.
```{r}
nr_labels <- which(!is.na(df[[1]]))
labels <- df[[1]][nr_labels]
labels
labels <- rep(labels,each = 3)
```

Remove comment rows by selecting rows with NA in first column
```{r}
df <- df[which(is.na(df[[1]])), ]
```

Next construct lowercase rownames from columns soort and kt
```{r}

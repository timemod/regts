...
### Functions `select_cols_by_name` and `drop_cols_by_name`

The function `select_cols_by_name` provides an alternative method to select
columns by name:
```{r, results = 'hide'}
select_cols_by_name(rts, c("a", "c")) # equivalent to rts[, c("a", "b)]
```

The advantage of using function is that you use it in a pipe:
```{r, results = 'hide'}
rts |>
  select_cols_by_name(c("a", "c"))
```

To remove specific columns use function `drop_col_by_name`:
```{r}
drop_cols_by_name(rts, c("c", "x")) # remove columns c and x
```

### Selecting or dropping columns with a regular expression
...
Function `drop_columns` can be used to remove columns based on a regular 
expressions. Example:
```{r}
drop_columns(rts, "1$") # drop all columns with names ending with '1'.
```
...
The result of period selection with the `[` operator 
is a `regts` object. If the `[[` operator is used instead, the 
result is a plain vector, which is sometimes useful, as discussed in
...
For multivariate timseries, the `[[` operator returns a plain matrix without
regts attributes. Example:
```{r}
rts[["2011Q2/2011Q4", c("a", "b")]]
```

## Function `select_range`

The function `select_range` provides an alternative method to select
periods. It is equivalent to using the `[` operator.
The advantage of using a function is that you can use it in a pipe:
```{r}
rts |>
  select_range("2011Q2/2011Q4")
```
This is often more readable than using the selection operator `[]`,
especially when combined with other functions for manipulating timeseries.
...
## Operators for `regts` \label{operators}

Since the `regts` class is a subclass of the `ts` class it inherits all
...
This behaviour of arithmetic operators may sometimes lead to unexpected results.
Suppose we want to compute an index series from `tx` using the following 
statement:
 
```{r}
tx_i <- 100 * tx / tx["2017q1"]
tx_i
```

The result is a timeseries with a single period, 2017Q1, because this is the
intersection of the periods of the two operands of the division operator (`/`).
In this case, use the `[[`
operator (see Section \ref{periodselection}) 
to extract the value of `tx` at 2017Q1 as a plain numerical value:
```{r}
tx_i <- 100 * tx / tx[["2017q1"]]
tx_i
```

## Functions for `regts` \label{regts_functions}
...
```{r}
t1 <- regts(data1,
  start = "2017q1", names = c("child", "adult", "senior"),
...
```{r}
ts_data <- regts(matrix(1:8, ncol = 2),
  start = "2016q1", end = "2016q4",
...
function `as_data_frame`. Variable `ts` has been previously defined:
...
frame column names and the periods show up in column `"period"`. Of course the 

...
Since `regts` does not understand Dutch month names, we have to convert these
periods to a standard format. This is possible by using argument `period_fun` 
of function `read_ts_xlsx` or `read_ts_csv`. `period_fun` is a function that
converts a text to a format that `regts` recognises.
To convert the Dutch month names we can employ function `dmy` (day-month-year) of 
package `lubridate`. This function can converts texts such as `"1 mei 2019"` to a
`Date` object which can then be easily formatted to a standard period in 
...
period_fun <- function(per) {
  ...
  # use locale "Dutch". This work on Windows only (on Linux use "nl_NL.utf8").
...
name of the Dutch locale on Windows[^1]. Obviously this code only works if
the Dutch locale has been installed. 

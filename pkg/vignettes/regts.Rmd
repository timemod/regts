...
The `data frame` can then be converted to a `regts`. This conversion is described in section \ref{dataframe}.
In advance it may be necessary to do some processing (e.g. selecting and/or deleting (parts of) rows and columns). You can find some examples in section \ref{deviant}.
...
## Reading information from a deviant file: some examples \label{deviant}

Suppose we have the following (simplified) file:

```{r echo = FALSE, comment=""}
cat(readLines("deviant.txt"), sep = "\n")
```

```{r}
# read file and convert to data frame
df <- read_excel("deviant.xlsx", col_names = FALSE)
df <- as.data.frame(df)
df
```

```{r}
# select labels and remove label row
labels <- c(rep(df[1,2], 2), rep(df[1,4], 2))
df <- df[-1,]

# set column names
names <- c("", "unemp","unemp", "lb","lb")
colnames(df) <- paste(df[1,], names, sep = "_")
colnames(df)[1] <- "time"
df

# remove first row, convert to regts using the time column and add labels
df <- df[-1,]
rts <- as.regts(df, time_column = "time")
ts_labels(rts) <- labels
rts
```

And another file with extra comment lines in between:

```{r echo = FALSE, comment=""}
cat(readLines("deviantt.txt"), sep = "\n")
```

```{r}
# read file and convert to data frame
df <- read_excel("deviantt.xlsx")
df <- as.data.frame(df)

# remove comment rows (select rows with NA in first column)
df <- df[which(is.na(df[1])), ]
# construct lowercase rownames from columns kt and soort
rownames(df) <- tolower(paste(df$kt, df$soort, sep = "."))
df

#  remove redundant columns 1 to 4
df[1:4] <- NULL

# transpose data frame and convert to regts
dft <- transpose_df(df)
dft
rts <- as.regts(dft)
```

...
`read_ts_csv`. For example, consider the following file

```{r echo = FALSE, comment=""}
cat(readLines("period.csv"), sep = "\n")
```

`regts` only understands abbreviated month names, so `june 2019` is
not recognised as a period by `read_ts_xlsx`.
To read this file, we can specify a function that converts dates 
such as `"june 2019"` to strings such as `"2019M06"`, To convert the month 
name we can employ function `dmy` (day-month-year) of package `lubridate`. 
To use this function we must add a day to the periods (e.g. `"1 june 2019"`), 
as shown  in the example below:


```{r}
period_fun <- function(per) {
  # This function converts a period with format <month> <year> 
  # to a format recognised by read_ts_xlsx.
  
  # convert input to day-month-year (add day 1) so we can use function dmy
  per <- paste("1", per)
  
  date <- lubridate::dmy(per, locale = "C", quiet = TRUE)
  return(format(date, "%YM%m"))
}

# Example
period_fun("august 2019")
```

Function `dmy` also has language/country parameter (`locale`),  which must be 
set if the current locale is not English. Here we use the cross platform locale
`"C"` which supports English month names on most  platforms. On Windows, you
could also specify the  locale `"English'`. If the month names are in a different
language, you can easiy modify `period_fun` by chosing an appropriate locale
(e.g. `"Dutch"`).

With function `period_fun` we can read the file:
```{r results = "hide"}

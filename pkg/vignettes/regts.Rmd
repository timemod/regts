...
Some common used file types to store data in R are rds, csv or xls(x) files.
Inside R projects, preferably use R data files, such as rds files. Writing and reading R objects to and from rds files is fast, easy and efficient.
If data has to be exported, you can use for example csv or Excel files.

## `.rds` file
The `.rds` file, an R-readable binary file, is a flexible file type, you can 
write any single R object. If you want to write more objects you can combine them in a list and write that list to the file. 
Such an object can be stored by the `saveRDS` function in the `base` package.
...
An object saved in such a `.rds` file can be reloaded  by the `readRDS` function,
...
Functions `save` and `load` are available to save and restore single or multiple objects to and from a Rdata file. Disadvantage is that the `load` function has no `return` variable. If multiple objects are written on a file it is not always clear what the result of this `load` function is.
...
Notice that the `sep` argument is not needed, separator `;` (as well as others) is recognised by the function. By default the separator is determined automatically.
...
Sometimes data in a file needs to be processed before it can be converted to a timeseries. This happens for instance when text and numbers are mixed up or when the file contains weird information. 

In that case try reading information as a `data frame` type. Most functions in R for reading a csv or Excel file return a `data frame`. Examples: `read.csv`, `read.csv2`, `read_xlsx`, `read.xlsx` or `read.table`.

The `data frame` can then be converted to a `regts`. This conversion is described in section \ref{dataframe}.
In advance it may be necessary to do some processing (e.g. selecting and/or deleting (parts of) rows and columns).
...
In R a data frame is a much-used type for storing data tables or vectors. So it is useful to take a closer look at this data type, regarding a `regts` object. For instance how can we convert a `data frame` object to a `regts` and vice versa. We also take a look at how to transpose a data frame. In between we discuss how to handle labels when doing all this.

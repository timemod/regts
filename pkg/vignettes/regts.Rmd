...
combination with library `openxlsx`. This approach makes it possible to change the default font of the workbook or specify a cellstyles.

`write_`ts_sheet` can be used to write to multiple sheets to the same file. This is more efficiënt than repeatedly using `write_ts_xlsx` with the `append_sheet` parameter.
...
Almost all functions work exactly the same for `regts` and `ts`. The only exceptions are `cbind` (discussed below) and `aggregate` ( described in vignette _Aggregation for (growth) timeseries_).
...
Yet the extension for `regts` has special features: arguments `union` and `suffixes`. The argument `union` effects the period range. With `union = TRUE`, the default value, the union of the period ranges of the joined objects is taken, otherwise it is the `intersection`. An example:
...
* `updval` only use valid values in the second timeseries to update the first timeseries

Of course only the common columns in both timeseries are updated. Extra columns in the second timeseries are added to the result, depending on the method and the content of that columns.
...
xts[ ,"b"] <- xts[ ,"a"] + 1
xts[ ,"c"] <- xts[ ,"b"] / xts[ ,"a"]
xts[ ,"d"] <- lag_ts(xts[ ,"a"], 1)
xts
```
Note that when timeseries `d` is created with function `lag_ts` you don't have to specify the period.^[When using function `lag` from the `stats` package setting the lagged period is essential. Otherwise the `xts` period is used. 
...
in some cases this is handy. They are alternatives for the `lag` function in the `stats` package.
An example:

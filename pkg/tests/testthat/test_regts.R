...
test_that("constructor regts for univariate timeseries", {
  ...
  ts1 <- ts(matrix(1:10, ncol = 1), start = c(2010, 1), frequency = 4,
            names = "a")
  expect_identical(regts1, as.regts(ts1))
  expect_identical(as.ts(regts1), ts1)

  regts1 <- regts(1:10, start = "2010", end = "2012")
  expect_identical(regts1, as.regts(ts(1:10, start = 2010, end = 2012,
                                       frequency = 1)))

  expect_error(
    regts(1:10, start = "2010-1"),
...
test_that("period / column selection in multivariate timeseries", {
  ...
  expect_identical(
    regts1["2011Q1", "c"],
    as.regts(window(ts1[, "c"], start = c(2011, 1), end = c(2011, 1)))
  )

  expect_identical(
    regts1["2011Q1", c("b", "a")],
    as.regts(window(ts1, start = c(2011, 1), end = c(2011, 1)))[, c("b", "a"),
                                                                drop = FALSE]
  )

  expect_identical(
    regts1["2011"],
    as.regts(window(ts1, start = c(2011, 1), end = c(2011, 4), extend = TRUE))
  )

  expect_identical(
    regts1["2011Q1", c("a", "b")],
    as.regts(window(ts1, start = c(2011, 1), end = c(2011, 1), extend = TRUE)
             [, c("a", "b"), drop = FALSE])
  )
})
...
test_that("column selection in a timeseries with 1 row", {
  ...
  regts1 <- regts(matrix(1:3, nc = 3), start = "2010Q2",
                  names = c("a", "b", "c"))
  ts1 <- as.ts(regts1)
...
test_that("start and end/prd_range, univariate", {
  ...
  expect_identical(regts(rep(1:2, 5)),
                   regts(1:2, period = period_range("1", "10")))
})
...
test_that("start and end/prd_range, multivariate", {
  ...
  expect_identical(regts(data[1:2, ]),
                   regts(data, period = period_range("1", "2")))
  expect_identical(regts(data[1, , drop = FALSE]),
                   regts(data, start = 1, end = 1))
  expect_identical(
    regts(data[1, , drop = FALSE], start = 1, end = 2),
...
test_that("start and end/prd_range, multivariate (1 column)", {
  ...
  expect_identical(regts(data[1:2, , drop = FALSE]),
                   regts(data, start = 1, end = 2))
  expect_identical(regts(data[1, , drop = FALSE]),
                   regts(data, start = 1, end = 1))
  expect_identical(regts(data[1, , drop = FALSE]),
                   regts(data, period = period_range("1", "1")))
  expect_identical(
    regts(data[1, , drop = FALSE], start = 1, end = 2),
    regts(rbind(data[1, , drop = FALSE], data[1, , drop = FALSE]),
          start = 1, end = 2)
...
test_that("period selection exotic timeseries", {
  ...
  expect_identical(logi_ts["2013Q4/2014Q1"][, 1],
                   regts(c(NA, FALSE), period = "2013q4/2014q1"))
  expect_identical(logi_ts["2018Q1/2018Q2"][, 1],
                   regts(NA, period = "2018q1/2018q2"))

  expect_identical(txt_ts["2018Q1/2018Q2"][, 1],
                   regts(NA_character_, period = "2018q1/2018q2"))

  expect_identical(txt_ts["2014Q2/2014Q3"][, 1],
                   regts(c("y", NA), period = "2014q2/2014q3"))

...
test_that("as.regts.data.frame for multivariate yearly ts with labels", {
  df <- data.frame(periods = c(2015, 2016, 2017), a = 1:3, b = 4:6)
  ts_labels <- paste("Timeseries", c("a", "b"))
  labelled::var_label(df) <- c("", ts_labels)

  expected_labels <- c("", ts_labels)
  names(expected_labels) <- colnames(df)
  expect_identical(labelled::var_label(df, unlist = TRUE), expected_labels)
...
test_that("as.regts.data.frame for multivariate quarterly irregular ts", {
  df <- data.frame(a = 1:3, b = 4:6)
...
test_that("as.regts.data.frame for argument numeric, a one row data frame", {
  ...
  rownames(df) <- 2015
  ts1 <- as.regts(df)
  ts2 <- regts(matrix(c(1.1, 2.2), ncol = 2), start = "2015",
               names = c("awn", "bwn"))
  expect_identical(ts1, ts2)
})
...
test_that("as.regts.data.frame for argument numeric = FALSE", {
  ...
  # use as.numeric to create non integer values, ts1 has also non integer values
  ts2 <- regts(matrix(c("1", "2", "3", "4", "5", "6"), ncol = 2),
               start = "2015", names = c("awn", "bwn"))
  expect_identical(ts1, ts2)
})
...
test_that("as.regts.data.frame with invalid period texts", {
  ...
  msg <- paste(
    "NAs introduced by coercion.\nThe following texts could not be converted",
    "to numeric:\n\"x\""
  )
...
test_that("as.regts.data.frame for a tibble", {
  ...
  # We don't want error messages about "Setting row names on a tibble is
  # deprecated"
...
test_that("as.regts.data.frame for column names starting with a number", {
  ...
  expected_result <- regts(matrix(1:3, ncol = 1), start = "2015Q3",
                           names = "3")

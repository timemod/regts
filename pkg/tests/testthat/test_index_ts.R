...
test_that("base_period and index_value", {
  bp <- "2021Q1"
  expect_equal(index_ts(a, base_period = bp), (100 / 13)* a)
  expected_result  <- (100 / 13) * ab
  expected_result$b <- expected_result$a
  expect_identical(index_ts(ab, bp), expected_result)

  bp <- "2020"
  mean_2020 <- mean(a[bp])
  expect_equal(index_ts(a, base_period = bp), (100 / mean_2020) * a)
  ...
  mean_bp <- mean(a[bp])
  expect_equal(index_ts(a, base_period = bp,
...
test_that("errors", {
  ...
  expect_error(index_ts(a, base_period  = "2017Q1"), msg)
})

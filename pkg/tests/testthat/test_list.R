...
test_that("as.list and as.regts.list for uni-ts without colnames", {
  labels <- "Var a"
...
test_that("exotic timeseries types", {
  # character
  x <- regts(matrix(c("a", "b", "c", "d"), ncol = 2), names = c("x", "y"),
             start = "2010", labels = c("Var x", "Var y"))
  l <- as.list(x)
  expect_identical(as.list(l), list(x = x$x, y = x$y))
  expect_equal(ts_labels(l[[2]]), "Var y")

  # logicals
  x <- regts(matrix(c(TRUE, FALSE, NA, TRUE), ncol = 2), names = c("x", "y"),
             start = "2010", labels = c("Var x", "Var y"))
  expect_identical(as.list(x), list(x = x$x, y = x$y))

  # complex numbers
  x <- regts(matrix(c(2 + 3i, 4, -1 + 0.25i, 3i), ncol = 2),
             names = c("x", "y"), start = "2010", labels = c("Var x", "Var y"))
  expect_identical(as.list(x), list(x = x$x, y = x$y))

  # illegal list type
  x <- regts(matrix(list(list(1, 2), list(3, 2), list(2, 9), list(9, 5)),
                    ncol = 2), names = c("x", "y"),
             start = "2010", labels = c("Var x", "Var y"))
